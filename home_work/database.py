...
def _format_output(data, col_list=None):
    """
    ...
    just_list = [col_list]
    for item in data:
        just_list.append([str(item[col] if item[col] is not None else '') for col in col_list])
    col_size = [max(map(len, col)) for col in zip(*just_list)]
    format_str = ' | '.join(["{{:<{}}}".format(i) for i in col_size])
    just_list.insert(1, ['-' * i for i in col_size])
...
in_data = ({"first_name": "Guido", "last_name": "Van Rossum",
            ...
            "birth": date(1969, 6, 28), "email": "iamanotido@python.org"})
